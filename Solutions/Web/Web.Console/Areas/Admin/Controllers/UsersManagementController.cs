...
{
    [Area("Admin")]
    public class UsersManagementController : ExtraAdminController
    {
        ...
        public IActionResult Index()
        {
            ...
            if (ViewData["DomainsSettingsList"] == null)
                ViewData["DomainsSettingsList"] = consoleBusiness.GetAllDomainsSettingsList(/*this.domainsSettings.UserIdCreator.Value*/);

            if (ViewData["RolesList"] == null)
                ViewData["RolesList"] = consoleBusiness.GetAllRolesList(/*this.domainsSettings.UserIdCreator.Value*/).Where(r=>!r.RoleName.Equals("Hosts")).ToList();
            ...
            bool? IsActivated = null)
        {
            try
            {
                try
                {
                    switch (DomainAndRole)
                    {
                        ...
                        case 2:// ملکاوان
                            ...
                            LevelIds.Add(consoleBusiness.GetLevelId("آگهی دهنده"));
                            break;
                        case 3://تنیاکو
                            RoleIds = new List<int>();
                            RoleIds.Add(2);
                            break;
                        case 4://سرمایه گذاران
                            RoleIds = new List<int>();
                            RoleIds.Add(3);
                            LevelIds = new List<int>();
                            LevelIds.Add(consoleBusiness.GetLevelId("سرمایه گذاران"));
                            break;
                        case 5://مشاورین املاک
                            RoleIds = new List<int>();
                            RoleIds.Add(3);
                            LevelIds = new List<int>();
                            LevelIds.Add(consoleBusiness.GetLevelId("مشاورین املاک"));
                            break;
                    }
                }
                catch (Exception exc)
                ...
                var usersList = consoleBusiness.GetUsersList(
                    ...
                    jtSorting,
                    //ParentId,
                    ((ParentId != 0) ? ParentId : this.parentUserId.Value),
                    //this.parentUserId.Value,
                    //this.userId.Value,
                    ((ParentId != 0) ? ParentId : this.userId.Value),
                    DomainSettingId,
                    //(DomainSettingId != 0) ? DomainSettingId : this.domainsSettings.DomainSettingId,
                    RoleIds,
                    LevelIds,
                    ((ParentId != 0) ? consoleBusiness.GetRoleIdsWithUserId(ParentId) : this.roleIds),
                    ((ParentId != 0) ? consoleBusiness.GetLevelIdsWithUserId(ParentId) : this.levelIds),
                    //this.roleIds,
                    //this.levelIds,
                    UserName,
                    Name,
        ...
        public IActionResult CreateUser()
        {
            ...
            {
                ViewData["DataBackUrl"] = "/Admin/UsersManagement/Index";
            }
        ...
        public IActionResult CreateUser(CustomUsersVM customUsersVM)
        {
            try
            {
                ...
                customUsersVM.CreateEnDate = DateTime.Now;
                customUsersVM.CreateTime = PersianDate.TimeNow;
                customUsersVM.UserIdCreator = this.userId.Value;
                ...
                if (ModelState.IsValid)
                {
                    ...
                    if (customUsersVM.UserId > 0)
                    {
                        ...
                        var domainSettings = consoleBusiness.GetDomainsSettingsWithDomainSettingId(customUsersVM.DomainSettingId, this.domainsSettings.UserIdCreator.Value);
        ...
        public IActionResult EditUser(long Id = 0)
        {
            ...
            if (!childsUsersIds.Contains(Id))
                return RedirectToAction("Index");

            //if (!consoleBusiness.GetChildUserIdsWithStoredProcedure(this.userId.Value).Contains(Id))
            //    return RedirectToAction("Index");
        ...
        public IActionResult EditUser(CustomUsersVM customUsersVM)
        {
            try
            {
                ...
                if (!customUsersVM.ParentUserId.HasValue)
                {
                    ...
                    {
                        customUsersVM.ParentUserId = consoleBusiness.GetParentUserIdWithUserId(this.userId.Value);
                    }
                    else
                    {
                        customUsersVM.ParentUserId = this.userId.Value;
                ...
                if (ModelState.IsValid)
                {
                    if (consoleBusiness.UpdateUser(customUsersVM, ref returnMessage))
                    {
                        return Json(new
                        {
                            Result = "OK",
                            UserId = customUsersVM.UserId,
                            Message = !string.IsNullOrEmpty(returnMessage) ? returnMessage : "با موفقیت ویرایش شد.",
        ...
        public IActionResult CompleteDeleteUser(int UserId = 0)
        {
            try
            {
                ...
                {
                    string serviceUrl = automationApiUrl + "/api/OrgChartNodesManagement/ExistOrgChartNodeWithUserId";

                    ExistOrgChartNodeWithUserIdPVM existOrgChartNodeWithUserIdPVM =
                        new ExistOrgChartNodeWithUserIdPVM()
                        {
                            UserId = UserId
                        };
                    ...
                    responseApiCaller = new AutomationApiCaller(serviceUrl).ExistOrgChartNodeWithUserId(existOrgChartNodeWithUserIdPVM);

                    if (responseApiCaller.IsSuccessStatusCode)
                    {
                        jsonResultObjectVM = responseApiCaller.JsonResultWithRecordsObjectVM;
                        ...
                        if (jsonResultObjectVM != null)
                        {
                            if (jsonResultObjectVM.Result.Equals("OK"))
                            {
                                ...
                                {
                                    Result = "ERROR",
                                    Message = "کاربر انتخاب شده در ساختار سازمانی وجود دارد"
                                });
                            }
                        }
                    }
                ...
                if (UserIds.Count > 0)
                {
                    ...
                    if (childsUserIds.Any(x => UserIds.Any(y => x.Equals(y))))
                    {
                        ...
                        foreach (var customUserVM in customUsersVM)
                        {
                            ...
                            {
                                var domainSettings = consoleBusiness.GetDomainsSettingsWithDomainSettingId(customUserVM.DomainSettingId, this.domainsSettings.UserIdCreator.Value);
        ...
        public IActionResult GetUsersList(int DomainSettingId, List<int> RoleIds)
        {
            try
            {
                ...
                ////////if (DomainSettingId == 0)
                ////////{
                ////////    usersList = consoleBusiness.GetUsersList(/*this.domainsSettings.DomainSettingId*/0, RoleId, this.userId.Value);
                ////////}
                ////////else
                ////////{
                ////////    domainAdminId = consoleBusiness.GetDomainAdminId(DomainSettingId);
                ////////    usersList = consoleBusiness.GetUsersList(DomainSettingId, RoleId, domainAdminId);
                ////////}
        ...
        public IActionResult GetLevelsList(int DomainSettingId, List<int> RoleIds, long UserId = 0)
        {
            try
            {
                ...
                if (UserId != 0)
                    levelsList = consoleBusiness.GetAllLevelsList(UserId, RoleIds/*DomainSettingId, RoleId, this.userId.Value*/);
                else
                if (DomainSettingId == 0)
                    levelsList = consoleBusiness.GetAllLevelsList(this.userId.Value, RoleIds/*DomainSettingId, RoleId, this.userId.Value*/);
                else
                    levelsList = consoleBusiness.GetAllLevelsList(consoleBusiness.GetDomainAdminId(this.domainsSettings.DomainSettingId), RoleIds/*DomainSettingId, RoleId, this.userId.Value*/);
        ...
        {
            ViewData["Title"] = "ساختار سلسله مراتبی";
        ...
        [AjaxOnly]
        [HttpPost]
        public IActionResult RemoveUserCertificate(long Id)
        {
            try
            {
                var userProfileCertificateCard = consoleBusiness.GetUserProfileCertificateCard(Id);
                var user = consoleBusiness.GetUserWithUserId(Id);
                var domainSetting = consoleBusiness.GetDomainsSettingsWithDomainSettingId(user.DomainSettingId);
                ...
                if (childsUSerIds.Contains(user.UserId))
                {
                    if (!string.IsNullOrEmpty(userProfileCertificateCard))
                    {
                        string userFolder = hostEnvironment.ContentRootPath + "\\wwwroot\\Files\\UserFiles\\";

                        if (System.IO.File.Exists(userFolder + "\\" + domainSetting.DomainName + "\\" + Id + "\\" + userProfileCertificateCard))
                        {
                            System.IO.File.Delete(userFolder + "\\" + domainSetting.DomainName + "\\" + Id + "\\" + userProfileCertificateCard);
                            System.Threading.Thread.Sleep(100);
                        }

                        //if (System.IO.File.Exists(userFolder + "\\" + domainSetting.DomainName + "\\" + Id + "\\thumb_" + userProfileCertificateCard))
                        //{
                        //    System.IO.File.Delete(userFolder + "\\" + domainSetting.DomainName + "\\" + Id + "\\thumb_" + userProfileCertificateCard);
                        //    System.Threading.Thread.Sleep(100);
                        //}

                        if (consoleBusiness.RemoveUserProfileCertificate(Id, this.userId.Value))
                            return Json(new
                            {
                                Result = "OK",
                                //Message = "Success"
        ...
        [AjaxOnly]
        [HttpPost]
        public IActionResult RemoveUserNationalCard(long Id)
        {
            try
            {
                var userProfileNationalCard = consoleBusiness.GetUserProfileNationalCard(Id);
                var user = consoleBusiness.GetUserWithUserId(Id);
                var domainSetting = consoleBusiness.GetDomainsSettingsWithDomainSettingId(user.DomainSettingId);
                ...
                if (childsUSerIds.Contains(user.UserId))
                {
                    if (!string.IsNullOrEmpty(userProfileNationalCard))
                    {
                        string userFolder = hostEnvironment.ContentRootPath + "\\wwwroot\\Files\\UserFiles\\";

                        if (System.IO.File.Exists(userFolder + "\\" + domainSetting.DomainName + "\\" + Id + "\\" + userProfileNationalCard))
                        {
                            System.IO.File.Delete(userFolder + "\\" + domainSetting.DomainName + "\\" + Id + "\\" + userProfileNationalCard);
                            System.Threading.Thread.Sleep(100);
                        }

                        //if (System.IO.File.Exists(userFolder + "\\" + domainSetting.DomainName + "\\" + Id + "\\thumb_" + userProfileNationalCard))
                        //{
                        //    System.IO.File.Delete(userFolder + "\\" + domainSetting.DomainName + "\\" + Id + "\\thumb_" + userProfileNationalCard);
                        //    System.Threading.Thread.Sleep(100);
                        //}

                        if (consoleBusiness.RemoveUserProfileNationalCard(Id, this.userId.Value))
                            return Json(new
                            {
                                Result = "OK",
                                //Message = "Success"
        ...
        [AjaxOnly]
        [HttpPost]
        public IActionResult RemoveUserResume(long Id)
        {
            try
            {
                var userProfileResume = consoleBusiness.GetUserProfileResume(Id);
                var user = consoleBusiness.GetUserWithUserId(Id);
                var domainSetting = consoleBusiness.GetDomainsSettingsWithDomainSettingId(user.DomainSettingId);
                ...
                if (childsUSerIds.Contains(user.UserId))
                {
                    if (!string.IsNullOrEmpty(userProfileResume))
                    {
                        string userFolder = hostEnvironment.ContentRootPath + "\\wwwroot\\Files\\UserFiles\\";

                        if (System.IO.File.Exists(userFolder + "\\" + domainSetting.DomainName + "\\" + Id + "\\" + userProfileResume))
                        {
                            System.IO.File.Delete(userFolder + "\\" + domainSetting.DomainName + "\\" + Id + "\\" + userProfileResume);
                            System.Threading.Thread.Sleep(100);
                        }

                        //if (System.IO.File.Exists(userFolder + "\\" + domainSetting.DomainName + "\\" + Id + "\\thumb_" + userProfileResume))
                        //{
                        //    System.IO.File.Delete(userFolder + "\\" + domainSetting.DomainName + "\\" + Id + "\\thumb_" + userProfileResume);
                        //    System.Threading.Thread.Sleep(100);
                        //}

                        if (consoleBusiness.RemoveUserProfileResume(Id, this.userId.Value))
                            return Json(new
                            {
                                Result = "OK",
                                //Message = "Success"
        ...
        [AjaxOnly]
        [HttpPost]
        public IActionResult RemoveUserContract(long Id)
        {
            try
            {
                var userProfileContract = consoleBusiness.GetUserProfileContract(Id);
                var user = consoleBusiness.GetUserWithUserId(Id);
                var domainSetting = consoleBusiness.GetDomainsSettingsWithDomainSettingId(user.DomainSettingId);
                ...
                if (childsUSerIds.Contains(user.UserId))
                {
                    if (!string.IsNullOrEmpty(userProfileContract))
                    {
                        string userFolder = hostEnvironment.ContentRootPath + "\\wwwroot\\Files\\UserFiles\\";

                        if (System.IO.File.Exists(userFolder + "\\" + domainSetting.DomainName + "\\" + Id + "\\" + userProfileContract))
                        {
                            System.IO.File.Delete(userFolder + "\\" + domainSetting.DomainName + "\\" + Id + "\\" + userProfileContract);
                            System.Threading.Thread.Sleep(100);
                        }

                        //if (System.IO.File.Exists(userFolder + "\\" + domainSetting.DomainName + "\\" + Id + "\\thumb_" + userProfileContract))
                        //{
                        //    System.IO.File.Delete(userFolder + "\\" + domainSetting.DomainName + "\\" + Id + "\\thumb_" + userProfileContract);
                        //    System.Threading.Thread.Sleep(100);
                        //}

                        if (consoleBusiness.RemoveUserProfileContract(Id, this.userId.Value))
                            return Json(new
                            {
                                Result = "OK",
                                //Message = "Success"
            ...
            bool? IsActivated = null)
        {
            try
            {
                ...
                var pageSize = consoleBusiness.CmsDb.Users.Count() + 1;
                int listCount = 0;
                ...
                foreach (var user in usersList)
                {
                    try
                    {
                        ...
                        row[nameFamilyOfUserCreatorTitle] = user.NameFamilyOfUserCreator.Replace("&nbsp;", " ");
        ...
        //{
        //    string filePath = "~/Content/excels/" + fileName + ".xls";
        //    if (System.IO.File.Exists(Server.MapPath(filePath)))
        //    {
        //        byte[] filedata = System.IO.File.ReadAllBytes(Server.MapPath(filePath));
        //        string contentType = MimeMapping.GetMimeMapping(Server.MapPath(filePath));
        ...
        private DataTable CreateDataTable()
        {
            ...
            DataColumn col = new DataColumn("نام کاربری", typeof(string));
            col.Caption = "نام کاربری";
            dt.Columns.Add(col);

            col = new DataColumn("رمز عبور", typeof(string));
            col.Caption = "رمز عبور";
            dt.Columns.Add(col);

            col = new DataColumn("نام و نام خانوادگی", typeof(string));
            col.Caption = "نام و نام خانوادگی";
            dt.Columns.Add(col);

            col = new DataColumn("سطح دسترسی", typeof(string));
            col.Caption = "سطح دسترسی";
            dt.Columns.Add(col);

            col = new DataColumn("نام کاربری مدیر مستقیم", typeof(string));
            col.Caption = "نام کاربری مدیر مستقیم";
            dt.Columns.Add(col);

            col = new DataColumn("نام و نام خانوادگی مدیر مستقیم", typeof(string));
            col.Caption = "نام و نام خانوادگی مدیر مستقیم";
            dt.Columns.Add(col);

            col = new DataColumn("آدرس", typeof(string));
            col.Caption = "آدرس";
            dt.Columns.Add(col);

            col = new DataColumn("شماره کارت", typeof(string));
            col.Caption = "شماره کارت";
            dt.Columns.Add(col);

            col = new DataColumn("موبایل", typeof(string));
            col.Caption = "موبایل";
            dt.Columns.Add(col);

            col = new DataColumn("کد ملی", typeof(string));
            col.Caption = "کد ملی";
            dt.Columns.Add(col);

            col = new DataColumn("شماره ثابت", typeof(string));
            col.Caption = "شماره ثابت";
            dt.Columns.Add(col);

            col = new DataColumn("تاریخ ثبت نام", typeof(string));
            col.Caption = "تاریخ ثبت نام";
            dt.Columns.Add(col);

            col = new DataColumn("آدرس شبکه اجتماعی", typeof(string));
            col.Caption = "آدرس شبکه اجتماعی";
            dt.Columns.Add(col);

            col = new DataColumn("شماره اختصاصی", typeof(string));
            col.Caption = "شماره اختصاصی";
            dt.Columns.Add(col);
        ...
        #region GetUserByUserName - old


        //[AjaxOnly]
        //[HttpPost]
        //public IActionResult GetUserByUserName(string username, string domainName)
        //{
        //    try
        //    {
        ...
        //        {
        //            var user = consoleBusiness.GetUserWithUserName(username, domainName);

        //            if (!user.UserId.Equals(0))
        //            {
        //                var userProfile = consoleBusiness.GetUsersProfile(user.UserId);

        //                if (userProfile != null)
        //                {
        //                    return Json(new { Result = "OK", Record = userProfile });
        //                }

        //            }
        //            else
        //            {
        //                return Json(new { Result = "OK", Record = string.Empty });
        //            }
        ...
        public IActionResult GetUserByUserName(string username, string domainName)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(username) || !string.IsNullOrEmpty(domainName))
                {
                    ...
                    if (consoleBusiness.CmsDb.Users.Where(c => c.UserName.Equals(username)).Any())  //اگر این کاربر در ملکاوان باشد
                    {
                    ...
                    else //اگر این کاربر در ملکاوان نباشد
                    {
                        if (consoleBusiness.CmsDb.UsersProfile.Where(c => c.Mobile.Equals(username) || c.Phone.Equals(username)).Any()) //اگر این کاربر در سیستم وجود داشته باشد
                        {

                            var user = consoleBusiness.CmsDb.UsersProfile.Where(f => f.Mobile.Equals(username) || f.Phone.Equals(username)).FirstOrDefault();

                            var levelId = consoleBusiness.CmsDb.Levels.Where(f => f.LevelName.Equals("آگهی دهنده")).Select(c => c.LevelId).FirstOrDefault();

                            var usersLevel = consoleBusiness.CmsDb.UsersLevels.Where(c => c.UserId.Equals(user.UserId) && c.LevelId.Equals(levelId)).FirstOrDefault(); //آن کاربری که دسترسی آگهی دهنده دارد

                            if (usersLevel != null)
                            {
                                if (usersLevel.UserId > 0)
                                {
        ...
        #region GetUserWithUserProfileMobile

        [AjaxOnly]
        [HttpPost]
        public IActionResult GetUserWithUserProfileMobile(string mobile)
        {
            try
            {
                if (!string.IsNullOrEmpty(mobile))
                {
                    var user = consoleBusiness.GetUserWithUserProfileMobile(mobile);
